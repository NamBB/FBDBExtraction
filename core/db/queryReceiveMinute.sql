...
from (Select MATCH.match_id
      ...
      where MATCH.match_id = GOAL.match_id
            and (MATCH.season_id = ?)
            and TEAM.name like ?
            and ((TEAM.team_id = MATCH.home_id and GOAL.minute > ? and GOAL.minute <= ? and GOAL.side = 1)
                or (TEAM.team_id = MATCH.away_id and GOAL.minute > ? and GOAL.minute <= ? and GOAL.side = 0))
